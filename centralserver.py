    ...
    def central_train(self):
        ...
        if self.interclusteringtype == "sync":
            ...
            for ind in participatingclusterind:
                ...
                messagequeue = Queue()
                t, _ = cluster.cluster_train(0, messagequeue)
                while not messagequeue.empty():
            ...
            for epoch in range(self.centralserverepochstart, self.centralserverepochend):
                for i in range(participatingclusternum):
                    ...
                    if self.args.interasyncthresholdexist==0 or staleness <=self.args.interasyncthreshold:
                    ...
                    messagequeue = Queue()
                    t,_ = self.clusterlist[pickedclusterind].cluster_train(modelversion,messagequeue)
